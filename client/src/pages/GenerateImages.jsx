    ...
    try {
      ...
      const prompt = `Generate an image of ${input} in the style ${selectedStyle}`
       const { data } = await axios.post(
        ...
        { prompt,publish }, // ✅ send only number
        {
   ...
   <div className="h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700">
      ...
      >
        ...
        <textarea
          ...
          placeholder="Describe the image you want to generate image..."
          required
        />
          ...
          {imageStyle.map((item) => (
            <span
              ...
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${
                selectedStyle === item
                  ? 'bg-green-50 text-green-600 border-purple-300'
        ...
        <button disabled={loading}
          type="submit"
          className="w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#00AD25] to-[#04FF50] text-white px-4 py-2 my-6 text-sm rounded-lg cursor-pointer hover:opacity-90 transition"
        >
